...
HGB_REG = dict(
    max_depth=6, max_iter=120, learning_rate=0.08, min_samples_leaf=40, random_state=42
)
HGB_CLF = dict(
    max_depth=5, max_iter=80, learning_rate=0.08, min_samples_leaf=40, random_state=42
)
...
CART_RATIO_FEATURES = [
    "cart_to_ord_ratio_7d",
    "cart_to_ord_ratio_30d",
    "abandoned_cart_7d",
    "abandoned_cart_30d",
]
...
def with_derived(df: pl.DataFrame) -> pl.DataFrame:
    ...
    out = df.with_columns(
        ...
        search_gmv_share_30d=pl.col("gmv_search_sum_30d")
        / (pl.col("gmv_sum_30d") + 1.0),
        gmv_per_ord_90d=pl.col("gmv_sum_90d") / (pl.col("to_ord_sum_90d") + 1.0),
        decay_gmv30=pl.col("gmv_sum_30d")
        / (pl.col("recency_days").fill_null(9999) + 1.0),
    )
    if "recency_order_days" in df.columns:
        out = out.with_columns(
            ord_lag=pl.col("recency_order_days").fill_null(9999)
            - pl.col("recency_days").fill_null(9999)
        )
    if "cart_to_ord_ratio_30d" not in out.columns:
        out = out.with_columns(
            cart_to_ord_ratio_7d=pl.col("to_cart_sum_7d")
            / (pl.col("to_ord_sum_7d") + 1.0),
            cart_to_ord_ratio_30d=pl.col("to_cart_sum_30d")
            / (pl.col("to_ord_sum_30d") + 1.0),
            abandoned_cart_7d=pl.when(
                pl.col("to_cart_sum_7d") > pl.col("to_ord_sum_7d")
            )
            .then(pl.col("to_cart_sum_7d") - pl.col("to_ord_sum_7d"))
            .otherwise(0.0),
            abandoned_cart_30d=pl.when(
                pl.col("to_cart_sum_30d") > pl.col("to_ord_sum_30d")
            )
            .then(pl.col("to_cart_sum_30d") - pl.col("to_ord_sum_30d"))
            .otherwise(0.0),
        )
    return out
...
def _X(df: pl.DataFrame, cols: list[str]) -> np.ndarray:
    ...
    if (
        any(
            c in RATIO_FEATURES
            or c in CART_RATIO_FEATURES
            or c in DECAY_FEATURES
            or c == "ord_lag"
            for c in cols
        )
        and "intensity_30d" not in df.columns
    ):
...
def fit_scale_high(train_df: pl.DataFrame, q50: float, q90: float) -> ArmModel:
    ...
    for c in np.linspace(0.5, 1.2, 15):
        ...
        if s < best:
            best, best_c = s, float(c)
    return ArmModel(
        "scale_high", {"c_high": best_c, "q90": q90, "inner_rmsle_high": best}
    )
...
def _fit_channel(
    train_df: pl.DataFrame, cols: list[str], **hgb_extra
) -> dict[str, Any]:
...
def _fit_lgb_channel(
    train_df: pl.DataFrame, cols: list[str], seed: int, **head_kw
) -> dict[str, Any]:
...
def _pred_two_log_heads_eps(
    reg_s, reg_c, xs: np.ndarray, eps: float = 1.0
) -> np.ndarray:
...
def _prepare_btyd(
    df: pl.DataFrame, params: dict[str, float] | None = None
    ...
    work = attach_btyd(df)
    if params is None:
        params = bgnbd_moments(
            work["btyd_frequency"].to_numpy(), work["btyd_T"].to_numpy()
        )
...
def _prepare_h65_stack(
    ...
    *,
    ...
    need_rord: bool = False,
) -> tuple[pl.DataFrame, dict[str, float]]:
    """Attach BTYD + IPI + channel lags (+ optional v3/knn/intent/rord) for H65-family arms.
    ...
    Args:
        ...
        need_intent: Add INTENT_DYNAMICS_FEATURES.
        need_rord: Add RECENT_ORD_FEATURES.

    Returns:
        (annotated df, bgnbd params).

    Example:
        work, params = _prepare_h65_stack(train, need_intent=True, need_rord=True)
    """
    from ltv_data import (
        add_v3_funnel2_features,
        attach_channel_lags,
        attach_cohortknn,
        attach_intent_dynamics,
        attach_order_ipi,
        attach_recent_ord,
    )
    ...
    if need_intent:
        work = attach_intent_dynamics(work)
    if need_rord:
...
def _fit_hurdle_logmix_c0(
    ...
    *,
    seed: int = 42,
    extra_params: dict | None = None,
    clf_cols: list[str] | None = None,
) -> dict[str, Any]:
    """Classifier P(y>0) + channel log1p regressors on buyers only.

    Args:
        train_df: Labeled fit rows with features already attached.
        cols: Feature columns for μ regressors.
        seed: LGB seed for classifier and regressors.
        extra_params: Overrides for LGB_PARAMS (num_leaves, min_data_in_leaf, …).
        clf_cols: Optional feature list for the classifier only (H87).

    Returns:
        Payload with clf, reg_s, reg_c, cols, clf_cols.

    Example:
        payload = _fit_hurdle_logmix_c0(work, cols, extra_params={"num_leaves": 31})
    """
    import lightgbm as lgb

    y = train_df["y"].to_numpy()
    use_clf_cols = list(clf_cols) if clf_cols is not None else cols
    X = _X(train_df, use_clf_cols)
    clf_params = {
        **LGB_PARAMS,
        "objective": "binary",
    ...
    dclf = lgb.Dataset(
        X, label=(y > 0).astype(np.int32), feature_name=use_clf_cols, free_raw_data=False
    )
    ...
    return {
        "clf": clf,
        "reg_s": _fit_lgb_head(pos, cols, "y_search", seed, extra_params=reg_extra),
        "reg_c": _fit_lgb_head(pos, cols, "y_cat", seed, extra_params=reg_extra),
        "cols": cols,
        "clf_cols": use_clf_cols,
    }
...
def _fit_hurdle_logmix_3seed(
    train_df: pl.DataFrame, cols: list[str], seeds: tuple[int, ...] = (42, 43, 44)
) -> dict[str, Any]:
    """Bag of hurdle-logmix c=0 models over multiple seeds.

    Args:
        train_df: Labeled fit rows with features attached.
        cols: Feature columns.
        seeds: Booster seeds for each member.

    Returns:
        Payload with ``members`` list of single-seed hurdle payloads.

    Example:
        payload = _fit_hurdle_logmix_3seed(work, cols)
    """
    return {
        "members": [_fit_hurdle_logmix_c0(train_df, cols, seed=s) for s in seeds],
        "cols": cols,
    }
...
def _fit_catboost_hurdle_member(
    ...
    *,
    ...
    Args:
        train_df: Labeled fit rows with features attached.
        cols: Feature columns for regressors.
        seed: CatBoost random_seed.
        clf_cols: Optional separate feature columns for the binary classifier.

    Returns:
        Payload with clf, reg_s, reg_c, cols, clf_cols (same shape as LGB hurdle member).

    Example:
        mem = _fit_catboost_hurdle_member(work, cols, seed=44, clf_cols=clf_cols)
    """
    from catboost import CatBoostClassifier, CatBoostRegressor

    use_clf_cols = list(clf_cols) if clf_cols is not None else cols
    xs_clf = _X(train_df, use_clf_cols)
    y = train_df["y"].to_numpy()
    ...
    clf.fit(xs_clf, (y > 0).astype(np.int32))
    pos = train_df.filter(pl.col("y") > 0)
    xs_pos = _X(pos, cols)
    kw = dict(
        loss_function="RMSE",
    ...
    return {"clf": clf, "reg_s": reg_s, "reg_c": reg_c, "cols": cols, "clf_cols": use_clf_cols}
...
def _fit_hurdle_logmix_mixed(
    ...
    *,
    ...
    Example:
        payload = _fit_hurdle_logmix_mixed(work, cols, clf_cols=clf_cols)
    """
    members = [_fit_hurdle_logmix_c0(train_df, cols, seed=s, clf_cols=clf_cols) for s in seeds_lgb]
    members.append(_fit_catboost_hurdle_member(train_df, cols, seed=seed_cb, clf_cols=clf_cols))
...
def _pred_hurdle_logmix_3seed(payload: dict[str, Any], work: pl.DataFrame) -> np.ndarray:
    ...
    Example:
        pred = _pred_hurdle_logmix_3seed(payload, eval_work)
    """
    p_acc = None
    mu_acc = None
    n = len(payload["members"])
    for mem in payload["members"]:
        clf_cols = mem.get("clf_cols") or mem.get("cols") or payload["cols"]
        mu_cols = mem.get("cols") or payload["cols"]
        p_pos = np.clip(
            _clf_positive_proba(mem["clf"], _X(work, clf_cols)), 0.0, 1.0
        )
        mu = _pred_two_log_heads(mem["reg_s"], mem["reg_c"], _X(work, mu_cols))
...
def _fit_dual_channel_hurdle(
    train_df: pl.DataFrame, cols: list[str], *, seed: int = 42
) -> dict[str, Any]:
    """Independent search/catalog hurdle-logmix heads with c=0.

    Args:
        train_df: Labeled fit rows with features attached.
        cols: Feature columns.
        seed: Shared LGB seed for all four boosters.

    Returns:
        Payload with clf_s/reg_s and clf_c/reg_c.

    Example:
        payload = _fit_dual_channel_hurdle(work, cols)
    """
    import lightgbm as lgb
    ...
    y_s = train_df["y_search"].to_numpy()
    y_c = train_df["y_cat"].to_numpy()
    dclf_s = lgb.Dataset(
        X, label=(y_s > 0).astype(np.int32), feature_name=cols, free_raw_data=False
    )
    dclf_c = lgb.Dataset(
        X, label=(y_c > 0).astype(np.int32), feature_name=cols, free_raw_data=False
    )
    clf_s = lgb.train(clf_params, dclf_s, num_boost_round=LGB_ROUNDS)
    clf_c = lgb.train(clf_params, dclf_c, num_boost_round=LGB_ROUNDS)
    reg_kw = {
        "extra_params": {
            "min_data_in_leaf": 60,
            "feature_fraction": 0.8,
    ...
    pos_s = train_df.filter(pl.col("y_search") > 0)
    pos_c = train_df.filter(pl.col("y_cat") > 0)
    return {
        "clf_s": clf_s,
        "clf_c": clf_c,
        "reg_s": _fit_lgb_head(pos_s, cols, "y_search", seed, **reg_kw),
        "reg_c": _fit_lgb_head(pos_c, cols, "y_cat", seed, **reg_kw),
        "cols": cols,
    }


def _pred_dual_channel_hurdle(payload: dict[str, Any], work: pl.DataFrame) -> np.ndarray:
    """Sum of per-channel expm1(p * log1p(mu)) assemblies.

    Args:
        payload: From ``_fit_dual_channel_hurdle``.
        work: Feature table.

    Returns:
        Non-negative total GMV predictions.

    Example:
        pred = _pred_dual_channel_hurdle(payload, eval_work)
    """
    xs = _X(work, payload["cols"])
    p_s = np.clip(payload["clf_s"].predict(xs), 0.0, 1.0)
    p_c = np.clip(payload["clf_c"].predict(xs), 0.0, 1.0)
    mu_s = np.expm1(np.clip(payload["reg_s"].predict(xs), -1, 20))
    mu_c = np.expm1(np.clip(payload["reg_c"].predict(xs), -1, 20))
    pred_s = np.expm1(p_s * np.log1p(np.clip(mu_s, 0, None)))
    pred_c = np.expm1(p_c * np.log1p(np.clip(mu_c, 0, None)))
    return _clip(pred_s) + _clip(pred_c)
...
def fit_arm(name: str, train_df: pl.DataFrame, q50: float, q90: float) -> ArmModel:
    ...
    if name == "hgb_log1p":
        return ArmModel(
            "hgb_log1p",
            {"reg": fit_hgb(train_df, BASE_FEATURES), "cols": BASE_FEATURES},
        )
    if name == "channel_sum":
    ...
    if name == "channel_order_lag":
        cols = (
            BASE_FEATURES + GAP_FEATURES + RATIO_FEATURES + ORDER_FEATURES + ["ord_lag"]
        )
        return ArmModel("channel_sum", _fit_channel(train_df, cols, hgb_kw=DEEPER_L2))
    ...
    if name == "channel_mono":
        ...
        for i, c in enumerate(cols):
            if c.startswith("gmv_sum_") or c in (
                "hist_gmv",
                "gmv_search_sum_30d",
                "gmv_cat_sum_30d",
            ):
    ...
    if name == "lgb_btyd_cal":
        ...
        return _fit_btyd_lgb(
            train_df, CALENDAR_FEATURES, attach_calendar, {"need_calendar": True}
        )
    if name == "lgb_btyd_lags":
        from ltv_data import NESTED_LAG_FEATURES, add_nested_gmv_lags

        return _fit_btyd_lgb(
            train_df, NESTED_LAG_FEATURES, add_nested_gmv_lags, {"need_lags": True}
        )
    if name == "lgb_btyd_chbtyd":
        from ltv_data import CHANNEL_BTYD_FEATURES, attach_channel_btyd

        return _fit_btyd_lgb(
            train_df, CHANNEL_BTYD_FEATURES, attach_channel_btyd, {"need_ch_btyd": True}
        )
    if name == "lgb_btyd_ipi":
        from ltv_data import ORDER_IPI_FEATURES, attach_order_ipi

        return _fit_btyd_lgb(
            train_df, ORDER_IPI_FEATURES, attach_order_ipi, {"need_ipi": True}
        )
    if name == "lgb_ipi_cal":
        from ltv_data import (
            CALENDAR_FEATURES,
    ...
    if name == "lgb_h59_nested_lags":
        ...
        def _prep(d):
            return add_nested_gmv_lags(attach_channel_lags(attach_order_ipi(d)))

        return _fit_btyd_lgb(
            train_df,
            ORDER_IPI_FEATURES + CHANNEL_LAG_FEATURES + NESTED_LAG_FEATURES,
            _prep,
            {"need_ipi": True, "need_ch_lags": True, "need_lags": True},
        )
    if name == "lgb_h59_rord":
        from ltv_data import (
            CHANNEL_LAG_FEATURES,
            ORDER_IPI_FEATURES,
            RECENT_ORD_FEATURES,
        ...
        def _prep(d):
            return attach_recent_ord(attach_channel_lags(attach_order_ipi(d)))

        return _fit_btyd_lgb(
            train_df,
            ORDER_IPI_FEATURES + CHANNEL_LAG_FEATURES + RECENT_ORD_FEATURES,
            _prep,
            {"need_ipi": True, "need_ch_lags": True, "need_rord": True},
        )
    if name == "lgb_h59_cart_conv":
        from ltv_data import (
            CHANNEL_LAG_FEATURES,
            ORDER_IPI_FEATURES,
            attach_channel_lags,
        ...
        def _prep(d):
            return with_derived(attach_channel_lags(attach_order_ipi(d)))

        return _fit_btyd_lgb(
            train_df,
            ORDER_IPI_FEATURES + CHANNEL_LAG_FEATURES + CART_RATIO_FEATURES,
            _prep,
            {"need_ipi": True, "need_ch_lags": True},
        )
    if name == "lgb_h59_reg":
    ...
    if name == "lgb_h59_joint":
        ...
        def _prep(d):
            return add_nested_gmv_lags(
                attach_recent_ord(attach_channel_lags(attach_order_ipi(d)))
            )

        selected_extra = ["ord_days_30d", "gmv_lag2_30", "gmv_lag3_30"]
        return _fit_btyd_lgb(
            train_df,
            ORDER_IPI_FEATURES + CHANNEL_LAG_FEATURES + selected_extra,
            _prep,
            {
                "need_ipi": True,
                "need_ch_lags": True,
    ...
    if name == "blend_h59_hgb":
        ...
        for seed in (42, 7, 99):
            hgb_m.append(
                _fit_channel(work, cols, hgb_kw={**H26_HGB, "random_state": seed})
            )
        return ArmModel(
            "blend_h59_hgb",
            {"lgb": lgb_m, "hgb": hgb_m, "bgnbd": params, "cols": cols},
        )
    if name == "lgb_ipi_bucket_c":
        ...
        for b, mask in buckets.items():
            cs[b] = (
                1.0
                if mask.sum() < 100
                else float(np.clip(_best_c(y[mask], pred[mask], grid), 0.8, 1.2))
            )
        return ArmModel(
            "lgb_ipi_bucket_c",
    ...
    if name == "lgb_ipi_logeps":
        ...
        members = [
            _fit_lgb_channel(work, cols, seed, y_transform="log1p_eps")
            for seed in (42, 7, 99)
        ]
        return ArmModel(
            "lgb_channel_ens",
    ...
    if name == "lgb_btyd_chlag":
        ...
        return _fit_btyd_lgb(
            train_df, CHANNEL_LAG_FEATURES, attach_channel_lags, {"need_ch_lags": True}
        )
    ...
    if name == "lgb_btyd_rord":
        ...
        return _fit_btyd_lgb(
            train_df, RECENT_ORD_FEATURES, attach_recent_ord, {"need_rord": True}
        )
    ...
    if name == "single_head_ens":
        ...
        for seed in (42, 7, 99):
            kw = {**H26_HGB, "random_state": seed}
            members.append(
                {"reg": fit_hgb(train_df, H26_COLS, "y", hgb_kw=kw), "cols": H26_COLS}
            )
    ...
    if name == "residual_mid_order":
        ...
        hist = train_df["hist_gmv"].to_numpy()
        ro = (
            train_df["recency_order_days"].fill_null(9999).to_numpy()
            if "recency_order_days" in train_df.columns
            else np.full(len(hist), 9999.0)
        )
        ...
        reg = HistGradientBoostingRegressor(
            **{**H26_HGB, "random_state": 42, "max_iter": 120}
        )
        if mask.sum() >= 1000:
        ...
        return ArmModel(
            "residual_mid_order",
            {"base": base, "resid": reg, "q50": q50, "q90": q90, "cols": H26_COLS},
        )
    if name == "channel_leaf50":
        ...
        for seed in (42, 7, 99):
            members.append(
                _fit_channel(train_df, cols, hgb_kw={**kw, "random_state": seed})
            )
        return ArmModel("channel_ens", {"members": members})
    if name == "lgb_total":
    ...
    if name == "rfm_te":
        ...
        return ArmModel(
            "rfm_te",
            {"members": members, "te_map": te_map, "edges": edges, "cols": cols},
        )
    if name == "vol_burst":
    ...
    if name == "time_decay_w":
        ...
        w = np.array(
            [
                np.exp(-(inner - (c.date() if hasattr(c, "date") else c)).days / 21.0)
                for c in cuts
            ]
        )
        members = [
            _fit_lgb_channel(train_df, H26_COLS, seed, weight=w) for seed in (42, 7, 99)
        ]
        return ArmModel("lgb_channel_ens", {"members": members})
    if name == "isotonic_log":
        ...
        acc = None
        for mem in members:
            part = _pred_two_log_heads(
                mem["reg_s"], mem["reg_c"], _X(train_df, H26_COLS)
            )
        ...
        iso.fit(
            ...
            np.log1p(np.clip(train_df["y"].to_numpy(), 0, None)),
        )
    ...
    if name == "blend_lgb_hgb":
        ...
        for seed in (42, 7, 99):
            hgb_m.append(
                _fit_channel(
                    train_df, H26_COLS, hgb_kw={**H26_HGB, "random_state": seed}
                )
            )
    ...
    if name == "channel_ratios_l2":
        ...
        return ArmModel(
            "channel_sum",
            _fit_channel(train_df, cols, hgb_kw=dict(l2_regularization=1.0)),
        )
    ...
    if name == "channel_poisson":
        ...
        extra = dict(
            ...
            y_transform="identity",
            hgb_kw=dict(
                max_iter=120, max_depth=6, learning_rate=0.08, min_samples_leaf=40
            ),
        )
    ...
    if name == "hurdle_logmix_3seed":
        work, params = _prepare_h65_stack(train_df)
        cols = _h65_feature_cols()
        payload = _fit_hurdle_logmix_3seed(work, cols, seeds=(42, 43, 44))
        return ArmModel(
            "hurdle_logmix_3seed",
            {
                **payload,
    ...
    if name == "dual_channel_hurdle":
        work, params = _prepare_h65_stack(train_df)
        cols = _h65_feature_cols()
        payload = _fit_dual_channel_hurdle(work, cols, seed=42)
        return ArmModel(
            "dual_channel_hurdle",
            {
                **payload,
    ...
    if name in ("stack_h65_hurdle3seed", "stack_h65_hurdle3seed_w85"):
        work, params = _prepare_h65_stack(train_df)
        cols = _h65_feature_cols()
        extra_kw = {
            "extra_params": {
        ...
        blend_w, hurdle_w = (0.15, 0.85) if name.endswith("_w85") else (0.30, 0.70)
        return ArmModel(
            "stack_blend_hurdle3",
            {
                "blend_members": members,
    ...
    if name == "stack_h65_hurdle4_mixed":
        work, params = _prepare_h65_stack(train_df)
        cols = _h65_feature_cols()
        extra_kw = {
            "extra_params": {
        ...
        hurdle = _fit_hurdle_logmix_mixed(
            work, cols, seeds_lgb=(42, 43, 44), seed_cb=45
        )
        return ArmModel(
            "stack_blend_hurdle3",
            {
    ...
    if name == "blend_h78_h82":
        ...
        hurdle_h78 = _fit_hurdle_logmix_3seed(work, cols, seeds=(42, 43, 44))
        hurdle_h82 = _fit_hurdle_logmix_mixed(
            work, cols, seeds_lgb=(42, 43), seed_cb=44
        )
        return ArmModel(
            "log_blend_h78_h82",
            {
                "blend_members": members,
                "hurdle_h78": hurdle_h78,
                "hurdle_h82": hurdle_h82,
                "w_h78": 0.70,
                "w_h82": 0.30,
                "stack_blend_w": 0.30,
                "stack_hurdle_w": 0.70,
    ...
    if name == "stack_h65_hurdle3_multidepth":
        work, params = _prepare_h65_stack(train_df)
        cols = _h65_feature_cols()
        extra_kw = {
            "extra_params": {
        ...
        hurdle = {
            "members": [
                _fit_hurdle_logmix_c0(work, cols, seed=s, extra_params=cfg)
                for s, cfg in depth_cfgs
            ],
            "cols": cols,
        }
        return ArmModel(
    ...
    if name == "stack_h65_hurdle3_clf_intent":
        from ltv_data import INTENT_DYNAMICS_FEATURES
    ...
    if name == "stack_h87_hurdle4_mixed":
        from ltv_data import INTENT_DYNAMICS_FEATURES
    ...
    if name == "stack_h87_mu_rord":
        from ltv_data import INTENT_DYNAMICS_FEATURES, RECENT_ORD_FEATURES

        work, params = _prepare_h65_stack(train_df, need_intent=True, need_rord=True)
        base_cols = _h65_feature_cols()
        clf_cols = _h65_feature_cols(INTENT_DYNAMICS_FEATURES)
        mu_cols = _h65_feature_cols(RECENT_ORD_FEATURES)
        extra_kw = {
            "extra_params": {
                "min_data_in_leaf": 60,
                "feature_fraction": 0.8,
        ...
        members = [
            _fit_lgb_channel(work, base_cols, seed, **extra_kw) for seed in (42, 7, 99)
        ]
        hurdle = {
            "members": [
                _fit_hurdle_logmix_c0(work, mu_cols, seed=s, clf_cols=clf_cols)
                for s in (42, 43, 44)
            ],
            "cols": mu_cols,
        }
    ...
    if name == "stack_h87_dual_routing_cb":
        from ltv_data import INTENT_DYNAMICS_FEATURES, RECENT_ORD_FEATURES
        ...
        hurdle_members.append(
            _fit_catboost_hurdle_member(work, mu_cols, seed=45, clf_cols=clf_cols)
        )
        hurdle = {
            "members": hurdle_members,
            "cols": mu_cols,
...
def predict_arm(model: ArmModel, eval_df: pl.DataFrame) -> np.ndarray:
    ...
    if name == "lgb_channel_ens":
        ...
        for mem in p["members"]:
            xs = _X(work, mem["cols"])
            if eps:
                part = _pred_two_log_heads_eps(
                    mem["reg_s"], mem["reg_c"], xs, float(eps)
                )
            else:
    ...
    if name == "hurdle_logmix_3seed":
        ...
        return _pred_hurdle_logmix_3seed(p, work)
    if name == "dual_channel_hurdle":
        work, _ = _prepare_h65_stack(
            eval_df,
            p.get("bgnbd"),
        ...
        return _pred_dual_channel_hurdle(p, work)
    if name == "stack_blend_hurdle":
        work, _ = _prepare_h65_stack(
            eval_df,
            p.get("bgnbd"),
    ...
    if name == "stack_blend_hurdle3":
        work, _ = _prepare_h65_stack(
            ...
            need_cohortknn=bool(p.get("need_cohortknn")),
            need_intent=bool(p.get("need_intent")),
            need_rord=bool(p.get("need_rord")),
        )
        if p.get("need_chbal"):
        ...
        for mem in p["blend_members"]:
            ...
            acc = part if acc is None else acc + part
        blend = acc / len(p["blend_members"])
        hurdle = _pred_hurdle_logmix_3seed(p["hurdle"], work)
        w_b = float(p.get("blend_w", 0.30))
        w_h = float(p.get("hurdle_w", 0.70))
        return _clip(w_b * blend + w_h * hurdle)
    if name == "log_blend_h78_h82":
        work, _ = _prepare_h65_stack(
            eval_df,
            p.get("bgnbd"),
            need_intent=bool(p.get("need_intent")),
        ...
        for mem in p["blend_members"]:
            ...
            acc = part if acc is None else acc + part
        blend = acc / len(p["blend_members"])
        w_b = float(p.get("stack_blend_w", 0.30))
        w_h = float(p.get("stack_hurdle_w", 0.70))
        y78 = _clip(w_b * blend + w_h * _pred_hurdle_logmix_3seed(p["hurdle_h78"], work))
        y82 = _clip(w_b * blend + w_h * _pred_hurdle_logmix_3seed(p["hurdle_h82"], work))
        a = float(p.get("w_h78", 0.70))
        b = float(p.get("w_h82", 0.30))
        return _clip(np.expm1(a * np.log1p(y78) + b * np.log1p(y82)))
    if name == "lgb_ipi_bucket_c":
    ...
    if name == "moe_hist":
        ...
        return _clip(pred)
    if name == "zero_snap":
        pred = _pred_two_log_heads(
            p["mem"]["reg_s"], p["mem"]["reg_c"], _X(eval_df, H26_COLS)
        )
        pred = pred.copy()
        pred[pred < p["tau"]] = 0.0
        return _clip(pred)
    ...
    if name == "isotonic_log":
        ...
        for mem in p["members"]:
            part = _pred_two_log_heads(
                mem["reg_s"], mem["reg_c"], _X(eval_df, H26_COLS)
            )
            acc = part if acc is None else acc + part
    ...
    if name == "blend_h59_hgb":
        from ltv_data import attach_channel_lags, attach_order_ipi
        ...
        for mem in p["hgb"]:
            part = predict_arm(ArmModel("channel_sum", mem), work)
            acc_h = part if acc_h is None else acc_h + part
        pred_lgb = np.clip(acc_l / len(p["lgb"]), 0, None)
        pred_hgb = np.clip(acc_h / len(p["hgb"]), 0, None)
        log_blend = 0.8 * np.log1p(pred_lgb) + 0.2 * np.log1p(pred_hgb)
        return _clip(np.expm1(log_blend))
    if name == "blend_lgb_hgb_btyd":
        work, _ = _prepare_btyd(eval_df, p["bgnbd"])
        cols = p["cols"]
        acc_l = None
    ...
    if name == "single_head_ens":
        ...
        for mem in p["members"]:
            part = np.expm1(
                np.clip(mem["reg"].predict(_X(eval_df, mem["cols"])), -1, 20)
            )
            acc = part if acc is None else acc + part
        return _clip(acc / len(p["members"]))
    if name == "mlp":
